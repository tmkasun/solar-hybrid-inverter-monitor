    ...
    for (const province of allCities) {
        ...
        for (const district of districts) {
            ...
            const response = await fetch(
                'https://fuel.gov.lk/api/v1/sheddetails/search',
                {
                    headers: {
            ...
            if (!response.ok) {
                ...
                if (data.length > 0) {
                    for (const shed of data) {
                        const { shedId } = shed;
                        const p92Response = await fetch(
                            `https://fuel.gov.lk/api/v1/sheddetails/${shedId}/p92`,
                            {
                                headers: {
                                    accept: 'application/json, text/plain, */*',
                                    'cache-control': 'no-cache',
                        ...
                        if (!p92Response.ok) {
                            ...
                            const p92Data = await p92Response.json();
                            if (allIds.has(shedId)) {
                                console.log('Duplicate ID: ', shedId);
                            } else {
                                allIds.add(shedId);
                                const thisProvince = allCities.find(
                                    (p) => p.provinceId === provinceId
                                );
                                if (!thisProvince) {
                                    console.log(
                                        `Province not found: ${provinceId}`
                                    );
                                    debugger;
                                }
                                const thisDistrict = thisProvince.district.find(
                                    (d) => d.districtId === districtId
                                );
                                if (!thisDistrict) {
                                    console.log(
                                        `thisDistrict not found: ${thisDistrict}`
                                    );
                                    debugger;
                                }
                                // thisDistrict.cities
