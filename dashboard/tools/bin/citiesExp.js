...
for (const province of allCities) {
    const { district: districts, id: randId, name: provinceName, provinceId, } = province;
    for (const district of districts) {
        const { cities, districtId, name: districtName } = district;
        for (const city of cities) {
            const { cityId, name } = city;
            newCities[`${name}#${cityId}`] = {
                cityId,
                name,
                districtId,
                districtName,
...
debugger;
//# sourceMappingURL=citiesExp.js.map