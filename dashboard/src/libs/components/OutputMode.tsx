...
export default function OutputMode() {
    ...
    const mutation = useMutation<any, any, string>(async (newMode) => {
        ...
        if (!response.ok) {
            throw new Error('Unable to change Inverter output mode');
        }
        const data = await response.json();
        if(data.accept) {
            return data;
        } else {
            throw new Error('Server has rejected the output change');
        }
    });
    ...
    return mutation.isLoading ? (
        ...
        <>
            <FormControl
                ...
                size="small"
            >
                <InputLabel id="inverter-charging-mode-label">Output</InputLabel>
                <Select
                ...
                >
                    ...
                    <MenuItem value={'SBU'}>Solar Battery Utility</MenuItem>
                    <MenuItem value={'UF'}>utility first</MenuItem>
                </Select>
            </FormControl>
            <Snackbar
