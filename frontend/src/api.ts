...
export const api = {
  ...
  history: () => request<{ samples: Array<Record<string, number | string | null>> }>("/api/history?hours=24"),
  ...
  diagnostics: () => request<{ diagnostics: Record<string, unknown>; latest: Status }>("/api/diagnostics")
};
