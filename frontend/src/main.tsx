...
function EnergyFlow({ values, connected }: { values: StatusValues; connected: boolean }) {
  ...
  const solarLine = "M 744 242 V 286 H 610 V 334";
  const gridLine = "M 175 426 H 396 L 460 378 H 522";
  const batteryLine = "M 566 373 V 407";
  const loadLine = "M 610 337 H 705 V 377 H 840";
  ...
  return <section className={`energy-flow panel ${isNight ? "night" : "day"}`} aria-label="Live energy flow">
    ...
    <div className="energy-diagram"><svg viewBox="0 0 1200 550" role="img" aria-label="House energy system: solar panels, CEB utility, battery and home load">
      ...
      <g className="source-label grid-label"><text x="56" y="458">CEB UTILITY</text><text x="56" y="480">{numeric(values.grid_voltage) ? `${numeric(values.grid_voltage).toFixed(0)} V · ${Math.abs(gridPower) > 1 ? watts(Math.abs(gridPower)) : "Standby"}` : "Unavailable"}</text></g><g className="source-label battery-label"><text x="515" y="475">BATTERY</text><text x="515" y="497">{values.battery_capacity_percent ?? "—"}% · {Math.abs(batteryPower) > 1 ? watts(Math.abs(batteryPower)) : "Idle"}</text></g>
      <g className="grid-asset" transform="translate(90 342)"><path d="M 0 84 H 85 M 23 0 V 84 M 61 0 V 84 M 12 24 H 72 M 12 49 H 72" /><circle cx="23" cy="0" r="4"/><circle cx="61" cy="0" r="4"/></g>
      <g className="house"><path className="house-shadow" d="M 432 458 H 1090"/><path className="house-wall" d="M 460 250 L 750 90 L 1055 255 V 458 H 460 Z"/><path className="house-roof" d="M 424 254 L 748 54 L 1090 250 L 1057 276 L 750 122 L 454 277 Z"/><path className="roof-panels" d="M 590 154 L 748 61 L 900 149 L 742 242 Z"/>{Array.from({ length: panelCount }, (_, index) => <rect key={index} className="roof-panel-cell" x={618 + (index % 6) * 34 + Math.floor(index / 6) * 14} y={143 + Math.floor(index / 6) * 22 - (index % 6) * 7} width="27" height="17" rx="1" />)}<path className="house-trim" d="M 460 250 L 750 90 L 1055 255 M 750 90 V 458"/><rect className="door" x="862" y="342" width="78" height="116" rx="3"/><rect className="window" x="650" y="302" width="62" height="58" rx="3"/><path className="window-lines" d="M 681 302 V 360 M 650 331 H 712"/><rect className="window" x="964" y="302" width="48" height="52" rx="3"/><path className="window-lines" d="M 988 302 V 354 M 964 328 H 1012"/>
        <g className="house-wires"><path className="wire-underlay" d={solarLine} /><path className={`wire solar-wire ${solarActive ? "active" : ""}`} d={solarLine} /><path className="wire-underlay" d="M 460 378 H 522" /><path className={`wire grid-wire ${gridActive ? "active" : ""} ${gridPower < 0 ? "reverse" : ""}`} d="M 460 378 H 522" /><path className="wire-underlay" d={batteryLine} /><path className={`wire battery-wire ${batteryActive ? "active" : ""} ${batteryPower < 0 ? "reverse" : ""}`} d={batteryLine} /><path className="wire-underlay" d={loadLine} /><path className={`wire load-wire ${loadActive ? "active" : ""}`} d={loadLine} /></g>
        <g className="wire-nodes"><circle className="solar-node" cx="744" cy="242" r="5"/><circle className="solar-node" cx="610" cy="334" r="5"/><circle className="grid-node" cx="460" cy="378" r="5"/><circle className="battery-node" cx="566" cy="407" r="5"/><circle className="load-node" cx="705" cy="377" r="5"/><circle className="load-node" cx="840" cy="377" r="5"/></g>
        <g className="wall-inverter" transform="translate(522 298)"><rect width="88" height="75" rx="8"/><circle cx="44" cy="30" r="15"/><path d="M 44 18 L 35 35 H 46 L 41 45 L 54 27 H 44 Z"/><text x="44" y="63">INVERTER</text></g><g className="battery-asset" transform="translate(515 407)"><rect width="102" height="38" rx="6"/><rect className="battery-level" width={Math.max(3, batteryPercent * .9)} height="26" x="5" y="6" rx="2"/><path d="M 103 11 H 110 V 27 H 103" /></g><text className="home-title" x="805" y="492">HOME USAGE</text><text className="home-value" x="805" y="516">{loadPower > 1 ? watts(loadPower) : "No load data"}</text></g>
      <g className="wire-label"><g className="wire-tag" transform="translate(634 260)"><rect x="-66" y="-16" width="132" height="28" rx="5"/><text>{solarActive ? `${watts(pvPower)} solar` : "Solar standby"}</text></g><g className="wire-tag" transform="translate(302 402)"><rect x="-76" y="-16" width="152" height="28" rx="5"/><text>{gridActive ? `${gridPower < 0 ? "Exporting" : "Importing"} ${watts(Math.abs(gridPower))}` : "Grid standby"}</text></g></g>
    </svg></div>
