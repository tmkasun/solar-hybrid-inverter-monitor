...
def parseQMOD(responseString):
    try:
        ...
        print("Error while parsing QMOD")
        return {"mode": "Unknow"}
...
def parseQPIGS(responseString):
    ...
    for parameter in inverterParameters:
        ...
        if currentIndex == 16:
            # 8 bit status flags
            deviceStatus = {
                "pvoac": {"name": "PV or AC feed the load", "value": parameter[0]},
                "confs": {"name": "Configuration status", "value": parameter[1]},
                "sccfwv": {"name": "SCC firmware version", "value": parameter[2]},
                "ls": {"name": "Load status", "value": parameter[3]},
                "res": {"name": "reserved", "value": parameter[4]},
                "conof": {"name": "Charging on/off", "value": parameter[5]},
                "sccconof": {"name": "SCC charging on/off", "value": parameter[6]},
                "acconof": {"name": "AC charging on/off", "value": parameter[7]},
            }
            parameter = deviceStatus
        elif currentIndex == 20:
            deviceStatus = {
                "fmode": {"name": "charging to floating mode", "value": parameter[0]},
                "sw": {"name": "Switch On", "value": parameter[1]},
                "dustp": {"name": "dustproof installed", "value": parameter[2]},
            }
            parameter = deviceStatus
        try:
            data[parameterMappings[currentIndex]] = parameter
        except:
            print("Error occored while parsing!")
        currentIndex += 1
    return data
